...
public class MarcasResource {
    ...
    public ResponseEntity<Marcas> createMarcas(@Valid @RequestBody Marcas marcas) throws URISyntaxException {
        ...
        if (marcas.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new marcas cannot already have an ID")).body(null);
        }
    ...
    @GetMapping("/marcas")
    @Timed
    public ResponseEntity<List<Marcas>> getAllMarcas(@ApiParam Pageable pageable) {
    ...
    @Timed
    public ResponseEntity<List<Marcas>> searchMarcas(@RequestParam String query, @ApiParam Pageable pageable) {
