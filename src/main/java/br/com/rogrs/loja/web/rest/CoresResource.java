...
public class CoresResource {
    ...
    public ResponseEntity<Cores> createCores(@Valid @RequestBody Cores cores) throws URISyntaxException {
        ...
        if (cores.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new cores cannot already have an ID")).body(null);
        }
    ...
    @GetMapping("/cores")
    @Timed
    public ResponseEntity<List<Cores>> getAllCores(@ApiParam Pageable pageable) {
    ...
    @Timed
    public ResponseEntity<List<Cores>> searchCores(@RequestParam String query, @ApiParam Pageable pageable) {
